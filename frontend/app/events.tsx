          ...
          filtered.map((e) => (
            <TouchableOpacity key={e.id} activeOpacity={0.92} style={styles.eventCard} testID={`event-${e.id}`}>
              <ImageBackground source={{ uri: e.image }} style={styles.eventImg} imageStyle={{ borderTopLeftRadius: radius.xl, borderTopRightRadius: radius.xl }}>
                ...
                <View style={styles.catPill}><Text style={styles.catTxt}>{e.category.toUpperCase()}</Text></View>
          ...
          <View>
            {certificates.map((c) => (
              <View key={c.id} style={styles.certCard}>
                <LinearGradient colors={mode === "dark" ? (["#3F2410", "#2D1A0A"] as const) : (["#FEF3C7", "#FDE68A"] as const)} style={styles.certRibbon}>
                  <Ionicons name="ribbon" size={22} color={mode === "dark" ? "#FDBA74" : "#92400E"} />
                </LinearGradient>
                <View style={{ flex: 1 }}>
                  <Text style={styles.certTitle}>{c.title}</Text>
                  <Text style={styles.certMeta}>Issued by {c.issuer} · {c.date}</Text>
                </View>
                <TouchableOpacity style={styles.dlBtn}><Ionicons name="download-outline" size={18} color={colors.primary} /></TouchableOpacity>
              </View>
            ))}
          </View>
        )}
...
function createStyles(colors: any, shadow: any, mode: "light" | "dark") {
  return StyleSheet.create({
    ...
    countdownLbl: { color: "#64748B", fontSize: 9, fontWeight: "800", letterSpacing: 1 },
    ...
    certCard: { flexDirection: "row", gap: 14, alignItems: "center", backgroundColor: colors.surface, padding: 14, borderRadius: radius.lg, marginBottom: 12, ...shadow.soft, borderWidth: mode === "dark" ? 1 : 0, borderColor: colors.border },
    ...
    dlBtn: { width: 40, height: 40, borderRadius: 20, backgroundColor: colors.surfaceAlt, alignItems: "center", justifyContent: "center" },
  });
}

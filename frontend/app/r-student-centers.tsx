...
import type { StudentCenterRow } from "../src/api/types";

export default function RStudentCenters() {
  const { token } = useAuth();
  const { colors, shadow, mode } = useTheme();
  const styles = useMemo(() => createStyles(colors, shadow, mode), [colors, shadow, mode]);

  const { data, loading, error } = useApi(
    () => (token ? api.reportStudentCenters() : Promise.resolve<StudentCenterRow[]>([])),
    [token]
  );

  return (
    <View style={styles.root} testID="rep-student-centers">
      <ReportScaffold<StudentCenterRow>
        ...
        renderItem={(item) => (
          <View style={styles.card}>
            ...
            <KV label="Type" value={item.centertype} />

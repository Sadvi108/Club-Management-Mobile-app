      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={colors.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.heroCard, shadow.strong]}>
          ...
          <View style={styles.heroRight}>
            ...
            <View style={styles.miniStats}>
              <View style={styles.mStat}><Text style={styles.mNum}>{attendanceData.present}</Text><Text style={styles.mLbl}>Present</Text></View>
              <View style={styles.mStat}><Text style={styles.mNum}>{attendanceData.total - attendanceData.present}</Text><Text style={styles.mLbl}>Missed</Text></View>
              <View style={styles.mStat}><Text style={styles.mNum}>{attendanceData.total}</Text><Text style={styles.mLbl}>Total</Text></View>
            </View>
          </View>
        </LinearGradient>
        ...
        <View style={styles.card}>
          ...
          <View style={styles.calRow}>
            {["S", "M", "T", "W", "T", "F", "S"].map((d, i) => (<Text key={i} style={styles.calHead}>{d}</Text>))}
          </View>
          {weeks.map((w) => (
...
function createStyles(colors: any, shadow: any, mode: "light" | "dark") {
  return StyleSheet.create({
    ...
    card: { backgroundColor: colors.surface, borderRadius: radius.xl, padding: 16, marginTop: 16, ...shadow.soft, borderWidth: mode === "dark" ? 1 : 0, borderColor: colors.border },

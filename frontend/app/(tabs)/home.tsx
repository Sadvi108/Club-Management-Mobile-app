        ...
        <LinearGradient colors={colors.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={styles.headerBg}>
          ...
          <View style={styles.statRow}>
            <View style={styles.stat}>
              <Text style={styles.statNum}>{student.attendance}%</Text>
              <Text style={styles.statLbl}>Attendance</Text>
            </View>
            <View style={styles.statSep} />
            <View style={styles.stat}>
              <Text style={styles.statNum}>{student.belt.split(" ")[0]}</Text>
              <Text style={styles.statLbl}>Current Belt</Text>
            </View>
            <View style={styles.statSep} />
            <View style={styles.stat}>
              <Text style={styles.statNum}>{student.fitness}</Text>
              <Text style={styles.statLbl}>Fitness</Text>
            </View>
          </View>
        </LinearGradient>
      </SafeAreaView>
      ...
      >
        ...
        <View style={styles.topQuick}>
          {[
            ...
            <TouchableOpacity
              key={q.id}
              testID={`quick-top-${q.id}`}
              style={styles.topQuickItem}
              onPress={() => router.push(q.route as any)}
              activeOpacity={0.7}
            >
              <View style={styles.topQuickIcon}>
        ...
        <View style={styles.grid}>
          {quickCards.map((c) => (
            <TouchableOpacity
              key={c.id}
              testID={`quick-card-${c.id}`}
              style={styles.gridCard}
              onPress={() => router.push(c.route as any)}
              activeOpacity={0.8}
            >
              <View style={[styles.gridIcon, { backgroundColor: c.color + "18" }]}>
...
const styles = StyleSheet.create({
  ...
  hi: { color: "rgba(255,255,255,0.8)", fontSize: 12 },
  name: { color: "#fff", fontSize: 18, fontWeight: "800", letterSpacing: -0.3 },
  badgeRow: { flexDirection: "row", alignItems: "center", gap: 4, marginTop: 4, backgroundColor: "rgba(255,255,255,0.18)", paddingHorizontal: 8, paddingVertical: 3, borderRadius: 10, alignSelf: "flex-start" },
  badgeTxt: { color: "#FDE68A", fontSize: 10, fontWeight: "700", letterSpacing: 0.3 },
  bell: { width: 42, height: 42, borderRadius: 21, backgroundColor: "rgba(255,255,255,0.18)", alignItems: "center", justifyContent: "center" },
  dot: { position: "absolute", top: 10, right: 10, width: 8, height: 8, borderRadius: 4, backgroundColor: "#F59E0B", borderWidth: 2, borderColor: colors.primary },
  statRow: { flexDirection: "row", marginTop: 22, backgroundColor: "rgba(255,255,255,0.14)", borderRadius: radius.lg, paddingVertical: 14 },
  stat: { flex: 1, alignItems: "center" },
  statNum: { color: "#fff", fontSize: 18, fontWeight: "800" },
  statLbl: { color: "rgba(255,255,255,0.8)", fontSize: 10, marginTop: 2, letterSpacing: 0.5 },
  statSep: { width: 1, backgroundColor: "rgba(255,255,255,0.2)" },

  topQuick: { flexDirection: "row", backgroundColor: "#fff", marginHorizontal: spacing.xl, marginTop: -20, borderRadius: radius.xl, paddingVertical: 16, paddingHorizontal: 8, ...shadow.card, justifyContent: "space-around" },
  ...
  dueLbl: { color: "#991B1B", fontSize: 11, fontWeight: "700", letterSpacing: 0.5 },
  dueAmt: { color: "#7F1D1D", fontSize: 24, fontWeight: "800", marginTop: 2 },
  dueDate: { color: "#B91C1C", fontSize: 11, marginTop: 2, fontWeight: "500" },
  dueBtn: { flexDirection: "row", gap: 6, alignItems: "center", backgroundColor: "#DC2626", paddingHorizontal: 16, paddingVertical: 10, borderRadius: radius.md },
  ...
  todayCard: { marginHorizontal: spacing.xl, backgroundColor: "#fff", borderRadius: radius.lg, padding: 14, flexDirection: "row", alignItems: "center", gap: 14, ...shadow.soft, overflow: "hidden" },
  ...
  checkInBtn: { flexDirection: "row", alignItems: "center", gap: 4, paddingHorizontal: 12, paddingVertical: 8, backgroundColor: "#EEF2FF", borderRadius: radius.sm },
  ...
  gridCard: { width: "22.5%", backgroundColor: "#fff", borderRadius: radius.lg, padding: 12, alignItems: "center", ...shadow.soft },
  ...
  eventCountdownLbl: { fontSize: 9, color: colors.textSecondary, fontWeight: "700", letterSpacing: 1 },
  eventBottom: { padding: 16, marginTop: "auto" },
  eventCat: { color: "#FDE68A", fontSize: 10, fontWeight: "700", letterSpacing: 1.5 },

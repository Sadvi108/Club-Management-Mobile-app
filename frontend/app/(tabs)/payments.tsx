...
export default function Payments() {
  ...
  const history = useApi(
    () => api.receipts({ fromDate: range.fromDate, toDate: range.toDate }),
    []
  );
    ...
    setTimeout(
      () =>
        Alert.alert(
          ...
          `RM ${totalDue.toLocaleString()} via ${methods.find((m) => m.id === selected)?.label}. Online payment opens the FPX gateway (Payment/Initiate) — to be enabled for live transactions.`
        ),
      200
    );
  };
    ...
    <View style={styles.root}>
      <SafeAreaView edges={["top"]} style={{ backgroundColor: colors.background }}>
        <View style={styles.header}>
          ...
          <TouchableOpacity style={styles.iconBtn} onPress={() => history.reload()}>
      ...
      <ScrollView contentContainerStyle={{ padding: spacing.xl, paddingBottom: 120 }} showsVerticalScrollIndicator={false}>
        <LinearGradient colors={colors.gradient} start={{ x: 0, y: 0 }} end={{ x: 1, y: 1 }} style={[styles.dueCard, shadow.strong]}>
          <View style={styles.dueRow}>
            ...
            <Text style={styles.dueLbl}>OUTSTANDING DUE</Text>
          </View>
          {dues.loading ? (
            <ActivityIndicator color="#fff" style={{ alignSelf: "flex-start", marginVertical: 10 }} />
          ) : (
            <Text style={styles.dueAmt}>RM {totalDue.toLocaleString()}</Text>
          )}
          <Text style={styles.dueLabel}>{nextInvoice?.invoiceDescription || (invoices.length === 0 && !dues.loading ? "No pending invoices" : "")}</Text>
          <Text style={styles.dueDate}>{invoices.length > 0 ? `${invoices.length} invoice${invoices.length === 1 ? "" : "s"} pending` : ""}</Text>
          {invoices.length > 0 && (
        ...
        {invoices.length > 0 && (
          <>
            <Text style={styles.section}>Pending Invoices</Text>
            {invoices.slice(0, 8).map((inv, idx) => (
              <View key={inv.invoiceId ?? idx} style={styles.histCard} testID={`invoice-${inv.invoiceId}`}>
                <View style={[styles.histIcon, { backgroundColor: mode === "dark" ? "#7F1D1D" : "#FEE2E2" }]}>
                  <Ionicons name="time" size={18} color={colors.danger} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={styles.histLabel} numberOfLines={1}>{inv.invoiceDescription || inv.transactionType}</Text>
                  <Text style={styles.histMeta}>{inv.period} · {fmtDate(inv.invoiceDate)}</Text>
                </View>
                <View style={{ alignItems: "flex-end" }}>
                  <Text style={styles.histAmt}>RM {(inv.dueAmount || 0).toLocaleString()}</Text>
                  <Text style={[styles.receiptTxt, { color: colors.danger }]}>{inv.paymentStatus}</Text>
                </View>
              </View>
            ))}
          </>
        ...
        <View style={styles.sectionHead}>
          ...
          {history.loading && <ActivityIndicator color={colors.primary} />}
        </View>

        {history.error && <Text style={styles.errTxt}>{history.error}</Text>}
        {!history.loading && (history.data?.length ?? 0) === 0 && (
          <Text style={styles.emptyTxt}>No receipts found.</Text>
        )}

        {(history.data ?? []).slice(0, 50).map((p, idx) => (
          <View key={`${p.id}-${idx}`} style={styles.histCard} testID={`payment-${p.id}`}>
            <View style={styles.histIcon}><Ionicons name="checkmark" size={18} color={colors.success} /></View>
            <View style={{ flex: 1 }}>
              <Text style={styles.histLabel} numberOfLines={1}>{p.paymentMethod}</Text>
              <Text style={styles.histMeta}>{fmtDate(p.receiptDate)} · {p.tcName} · #{p.receiptNo}</Text>
            </View>
            <View style={{ alignItems: "flex-end" }}>
              <Text style={styles.histAmt}>RM {(p.receiptAmount || 0).toLocaleString()}</Text>
              <TouchableOpacity
                style={styles.receiptBtn}
                testID={`receipt-${p.id}`}
                onPress={() => Alert.alert("Receipt", `${p.paymentMethod}\nReceipt #${p.receiptNo}\nRM ${p.receiptAmount}\n${fmtDate(p.receiptDate)}`)}
              >
      ...
      <Modal visible={showPay} animationType="slide" transparent>
        <View style={styles.modalOverlay}>
          <View style={styles.modalCard}>
            ...
            <Text style={styles.modalAmt}>RM {totalDue.toLocaleString()}</Text>

...
class _ChatThreadScreenState extends State<ChatThreadScreen> {
  ...
  Future<void> _load() async {
    ...
    final userId = session.currentStudentId ?? 0;

    // A brand-new help desk conversation has no server side yet — it is only what we sent.
    List<dynamic> incoming = const [];
    if (!_isHelpdesk) {
      incoming = session.notifications ?? const [];
      if (incoming.isEmpty) {
        try {
          incoming = findRecordList(await Api.profileMyNotifications());
        } catch (e) {
  ...
  Future<void> _markRead(List<dynamic> incoming) async {
    for (final n in incoming) {
      ...
      try {
        ...
        n['isRead'] = true;
      } catch (_) {/* stays unread; not worth interrupting the user */}
    }
  }
  ...
  Future<void> _send() async {
    ...
    try {
      ...
      setState(() {
        _bubbles = [
          ..._bubbles,
          ChatBubble(key: 'out-${msg.id}', mine: true, text: msg.text, at: msg.at),
        ];
        _input.clear();
        _sending = false;
      });
        ...
        Expanded(
          child: _loading
              ...
              : _bubbles.isEmpty
                  ...
                  : ListView.builder(
                      controller: _scroll,
                      padding: const EdgeInsets.fromLTRB(Gaps.lg, Gaps.md, Gaps.lg, Gaps.md),
  ...
  Widget _empty(AppColors c) => Center(
        child: Padding(
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            Icon(Icons.forum_outlined, size: 40, color: c.textMuted),
      ...
      child: Container(
        ...
        child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
          if (!mine && b.subject != null) ...[
            Text(b.subject!,
                style: TextStyle(
                    color: c.primary, fontSize: 10.5, fontWeight: FontWeight.w800)),
